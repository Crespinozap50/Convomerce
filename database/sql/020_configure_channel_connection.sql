...
-- El token permanece fuera de PostgreSQL; solo se persiste su referencia opaca.
...
create function app.record_channel_connection_validation(
  _actor_user_id uuid, _connection_id uuid, _successful boolean, _error_code text
)
returns boolean
language plpgsql
security definer
...
as $$
begin
  if app.current_tenant_id() is null or not app.can_manage_channel_connections(_actor_user_id) then
    raise insufficient_privilege using message = 'Actor is not authorized to validate connections';
  end if;
  update app.channel_connections
     set status = case when _successful then 'connected' else 'error' end,
         last_validated_at = now(),
         last_error_code = case when _successful then null else coalesce(_error_code, 'UNKNOWN') end,
         updated_at = now()
   where tenant_id = app.current_tenant_id() and id = _connection_id;
  return found;
end
$$;

revoke all on function app.record_channel_connection_validation(uuid,uuid,boolean,text) from public;
grant execute on function app.record_channel_connection_validation(uuid,uuid,boolean,text) to commerce_runtime;
