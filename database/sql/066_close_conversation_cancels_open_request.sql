-- The project owner found a real leftover: a conversation closed (by
-- inactivity or manually) while the customer had an unfinished order in
-- progress — the commercial_request stayed stuck in 'draft'/
-- 'awaiting_confirmation' and its conversation_workflow stayed 'active'
-- forever, since neither close path ever touched them. Surfaced directly by
-- D-057's merge, which had to manually cancel exactly this kind of stale
-- draft to free up the one-editable-request-per-conversation slot.
--
-- Same cancellation shape already used everywhere else a request is
-- abandoned (commercial-flow.service.ts's own 'cancel' command,
-- appointment-flow.service.ts) — set commercial_requests.status='cancelled'
-- and conversation_workflows.status='cancelled', nothing more (no separate
-- hold-release step exists anywhere else in the codebase either).
