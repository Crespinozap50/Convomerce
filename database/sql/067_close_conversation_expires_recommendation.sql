-- Same class of bug as 066 (closing a conversation didn't cancel an
-- unfinished order/workflow), found by auditing for the same pattern
-- elsewhere: a recommendation shown to the customer (D-027) that never got
-- an explicit accept/reject stays status='shown' forever, even after the
-- conversation closes. If the customer reopens the conversation weeks later
-- and taps that stale button, RecommendationService.accept() would still
-- happily add it — the recommendation_events row was designed for exactly
-- this ('expired' is already a valid status in the check constraint, used
-- reactively by recommendationExpired copy, but nothing ever set it
-- proactively). Marks any 'shown' recommendation as 'expired' for whichever
-- conversations this same sweep just closed.
