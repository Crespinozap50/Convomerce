-- D-047 established close_inactive_conversations()'s per-tenant-loop
-- pattern for safe cross-tenant sweeps. This migration extends it per the
-- project owner's request: warn the customer before closing an inactive
-- human-handled conversation, then close it a few minutes later if still no
-- reply — instead of closing silently with no notice.
--
-- Two-stage timing: the warning fires once inactivity reaches
-- (conversation_timeout_minutes - grace_minutes); the actual close still
-- happens at the tenant's already-configured conversation_timeout_minutes,
-- same total wait as before this migration — the grace period is carved
-- out of the existing timeout, not added on top of it. grace_minutes is a
-- fixed 5 minutes for now, not yet per-tenant configurable; a tenant whose
-- timeout is at or below that skips the warning stage entirely and closes
-- directly, same as before this migration.
set role commerce_owner;

alter table app.conversations add column closing_warning_sent_at timestamptz;
