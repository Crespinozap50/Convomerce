...
// D-128: response_rewriting was the only purpose this service ever
...
export type AiUsagePurpose='response_rewriting'|'consultative_recommendation';
...
export class AiUsageBudgetService{
  ...
  // D-128: unlike NaturalResponseRewriter (always called after its domain
  // transaction has committed — see D-041 in docs/decisions.md), a
  // consultative recommendation IS the domain decision, so it has to run
  // from inside CommercialFlowService.resolve()'s still-open transaction.
  // Calling database.withTenantTransaction() there — a second connection,
  // a second transaction — reproduced D-041's exact deadlock: the new
  // connection's insert into app.ai_usage_reservations blocks on a
  // transactionid lock held by the still-open outer transaction (both
  // reference the same app.conversations row), and the outer transaction
  // never proceeds to commit because Node is awaiting this call first.
  // Reproduced live against the local DB with pg_locks evidence before
  // this fix. An optional `client` lets a caller that's already inside a
  // transaction reuse it instead — no second connection, nothing to
  // deadlock against. NaturalResponseRewriter never passes one, so its
  // existing own-transaction behavior (and the D-041 fix it depends on) is
  // untouched.
  reserve(context:AiRewriteContext,purpose:AiUsagePurpose='response_rewriting',client?:PoolClient):Promise<AiBudgetDecision>{
    if(client)return this.doReserve(client,context,purpose);
    return this.database.withTenantTransaction(context.tenantId,c=>this.doReserve(c,context,purpose));
  }

  private async doReserve(client:PoolClient,context:AiRewriteContext,purpose:AiUsagePurpose):Promise<AiBudgetDecision>{
    ...
    const reservation:AiBudgetReservation={id:uuidv7(),tenantId:context.tenantId,conversationId:context.conversationId,messageId:context.messageId,reservedCostMinor:reservedCost,currency:policy.cost_currency,purpose};
    await client.query(`insert into app.ai_usage_reservations(id,tenant_id,conversation_id,message_id,purpose,reserved_cost_minor) values($1,app.current_tenant_id(),$2,$3,$5,$4)`,[reservation.id,context.conversationId,context.messageId,reservedCost,purpose]);
    return{allowed:true,reservation};
  }

  settle(reservation:AiBudgetReservation,usage:AiUsageSettlement,client?:PoolClient):Promise<void>{
    if(client)return this.doSettle(client,reservation,usage);
    return this.database.withTenantTransaction(reservation.tenantId,c=>this.doSettle(c,reservation,usage));
  }

  private async doSettle(client:PoolClient,reservation:AiBudgetReservation,usage:AiUsageSettlement):Promise<void>{
    ...
    await client.query(`insert into app.ai_usage(id,tenant_id,conversation_id,message_id,provider,model,purpose,input_tokens,output_tokens,estimated_cost_minor,cost_currency,latency_ms,success) values($1,app.current_tenant_id(),$2,$3,$4,$5,$12,$6,$7,$8,$9,$10,$11)`,[uuidv7(),reservation.conversationId,reservation.messageId,usage.provider,usage.model,usage.inputTokens,usage.outputTokens,usage.actualCostMinor,reservation.currency,usage.latencyMs,usage.success,reservation.purpose]);
  }
