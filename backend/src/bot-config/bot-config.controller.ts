...
function parse(body:unknown):BotConfigInput{const x=body as Record<string,unknown>;if(!x||typeof x!=='object')throw badRequest('VALIDATION_ERROR','Invalid configuration');const s=(v:unknown,n:string)=>{if(typeof v!=='string'||!v.trim())throw badRequest('VALIDATION_ERROR',`${n} is required`);return v.trim()};const integer=(v:unknown,n:string,min:number,max:number)=>{if(!Number.isInteger(v)||Number(v)<min||Number(v)>max)throw badRequest('VALIDATION_ERROR',`${n} must be an integer between ${min} and ${max}`);return Number(v)};const locale=s(x.locale,'locale');if(!isValidLocale(locale))throw badRequest('VALIDATION_ERROR','locale must be a valid BCP 47 language tag');const policy=(x.aiResponsePolicy??{}) as Record<string,unknown>;const conversationTimeoutMinutes=x.conversationTimeoutMinutes===null||x.conversationTimeoutMinutes===undefined?null:integer(x.conversationTimeoutMinutes,'conversationTimeoutMinutes',1,10080);const messageRetentionDays=x.messageRetentionDays===null||x.messageRetentionDays===undefined?null:integer(x.messageRetentionDays,'messageRetentionDays',7,3650);
// D-140: per-tenant opt-in word/phrase shown before each category row in
// the picker ("Menú Tacos" for Santos Tacos) — NULL/empty means no prefix
// at all (CrediCel Store's "Celulares"). Capped well under WhatsApp's own
// 24-char row-title limit so there's still room left for the category
// name itself once the two are joined.
const categoryLabelPrefixRaw=x.categoryLabelPrefix;
if(categoryLabelPrefixRaw!==null&&categoryLabelPrefixRaw!==undefined&&typeof categoryLabelPrefixRaw!=='string')throw badRequest('VALIDATION_ERROR','categoryLabelPrefix must be a string or null');
const categoryLabelPrefixTrimmed=typeof categoryLabelPrefixRaw==='string'?categoryLabelPrefixRaw.trim():'';
if(categoryLabelPrefixTrimmed.length>30)throw badRequest('VALIDATION_ERROR','categoryLabelPrefix must be at most 30 characters');
const categoryLabelPrefix=categoryLabelPrefixTrimmed===''?null:categoryLabelPrefixTrimmed;
return {enabled:x.enabled===true,assistantName:s(x.assistantName,'assistantName'),locale:normalizeLocale(locale),welcomeMessage:s(x.welcomeMessage,'welcomeMessage'),fallbackMessage:s(x.fallbackMessage,'fallbackMessage'),handoffKeywords:Array.isArray(x.handoffKeywords)?x.handoffKeywords.filter(v=>typeof v==='string').map(v=>(v as string).trim()).filter(Boolean):[],conversationTimeoutMinutes,messageRetentionDays,categoryLabelPrefix,aiResponsePolicy:{enabled:policy.enabled===true,rolloutPercentage:integer(policy.rolloutPercentage??0,'aiResponsePolicy.rolloutPercentage',0,100),dailyRequestLimit:integer(policy.dailyRequestLimit??100,'aiResponsePolicy.dailyRequestLimit',0,100000),monthlyCostLimitMinor:integer(policy.monthlyCostLimitMinor??500,'aiResponsePolicy.monthlyCostLimitMinor',0,100000000)}}}
